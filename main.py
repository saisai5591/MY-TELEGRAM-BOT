...
# 1. 讀取 Render 環境變數 (確保 TOKEN 同 OPENAI_API_KEY 已填)
...
# 3. 遊戲指令 (全部拍齊左邊，解決 SyntaxError)
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("歡迎來到海龜湯！\n指令：/play 開始, /end 睇真相, /next 下一局")
...
async def end(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if 'game' in context.chat_data:
        ans = context.chat_data['game']['answer']
        await update.message.reply_text(f"【強制結束】答案係：\n{ans}")
        context.chat_data.clear()
    else:
        await update.message.reply_text("目前無進行緊遊戲。")

async def next_puzzle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    game = random.choice(puzzles)
    context.chat_data.update({'game': game, 'history': [], 'attempts': 20})
    await update.message.reply_text(f"【切換新湯】：\n{game['story']}\n\n你有 20 次提問機會！")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    system_prompt = (
        f"故事：{game['story']}\n"
        f"真相：{game['answer']}\n"
        "設定：你係專業海龜湯判官。如果玩家講中咗關鍵真相，請回覆『【遊戲結束：真相還原】』並詳細解說。"
        "否則，只答「係」、「唔係」或者「無關」。"
    )
    ...
    try:
        ...
        if "【遊戲結束：真相還原】" in reply:
            ...
            # 記錄對話歷史
            context.chat_data.setdefault('history', []).extend([
                {"role": "user", "content": user_text},
                {"role": "assistant", "content": reply}
            ])
            warning = f"⚠️ 只剩最後 {rem} 次機會！\n" if rem <= 2 else ""
            await update.message.reply_text(f"{warning}{reply}\n\n(餘：{rem})")

    except Exception as e:
        logging.error(f"API Error: {e}")
        await update.message.reply_text("AI 思考中斷，請重試。")

# 5. 啟動區 (解決 NameError，全部線路接駁正確)
